...
class FluxCore(AutotoolsPackage):
    ...
    def setup_build_environment(self, env):
        ...
        # Ensure LIBARCHIVE can be found during configure step
        if "libarchive" in self.spec:
            env.set("LIBARCHIVE_LIBS", f"-L{self.spec['libarchive'].libs.directories[0]} -larchive")
            env.set("LIBARCHIVE_CFLAGS", f"-I{self.spec['libarchive'].headers.directories[0]}")



    @run_after("setup_build_environment")
